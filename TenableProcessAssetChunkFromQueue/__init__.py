...
from ..tenable_helper import get_asset_export_url, TenableStatus
...
def main(msg: func.QueueMessage) -> None:
    ...
    try:
        ...
        if export_job_id == '' or chunk_id == '':
            ...
            logging.info(
                f'downloading chunk at {get_asset_export_url()}/{export_job_id}/chunks/{chunk_id}')
            tio = TenableIO(vendor=vendor, product=product, build=build)
            r = tio.get(
                f'{get_asset_export_url()}/{export_job_id}/chunks/{chunk_id}')
            logging.info(
                f'received a response from {get_asset_export_url()}/{export_job_id}/chunks/{chunk_id}')
