...
from ..tenable_helper import get_vuln_export_url
...
def main(timestamp: int) -> object:
    ...
    logging.info(
        f'sending {json.dumps(body)} to {get_vuln_export_url()}')
    r = tio.post(get_vuln_export_url(), data=json.dumps(body))
    logging.info(f'received a response from {get_vuln_export_url()}')
    logging.info(r.text)
    logging.info(r.json())
    return r.json()
